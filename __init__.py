...
@app.route("/menu")
def menu():
    image_folder = os.path.join(app.static_folder, 'images/menu/')
...
@app.route("/stage")
def stage():
    image_folder = os.path.join(app.static_folder, 'images/stage/')
    num_images = len([f for f in os.listdir(image_folder) if os.path.isfile(os.path.join(image_folder, f))])
    return render_template('stage.html', num_images=num_images)
